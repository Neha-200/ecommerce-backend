...
export const getLatestProducts = TryCatch(async (req,res,next) => {
    ...
    if (myCache.has("latest-products")) products = 
    JSON.parse(myCache.get("latest-products") as string);
    else {
...
export const getAllProducts = TryCatch(async (req:Request<{},{},{},
    SearchRequestQuery>,res,next) => {
    ...
    const [products, filtersProductOnly] = await Promise.all([
        productsPromise,
        Product.find(baseQuery)
    ]);

    const totalPage = Math.ceil( filtersProductOnly.length / limit );
