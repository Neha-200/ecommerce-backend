...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransactionsPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthOrders, thisMonthProducts, thisMonthUsers, lastMonthOrders, lastMonthProducts, lastMonthUsers, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUsersCount, latestTransaction] = await Promise.all([
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const allOrderPromise = Order.find({})
            .select(["total", "discount", "tax", "subtotal", "shippingCharges"]);
